...
import { 
  ArrowRight, MapPin, Calendar, Save, Loader2, Search, FileText, ChevronDown,
  ChevronUp
} from 'lucide-react';
...
import LineItemsEditor from "@/components/job/LineItemsEditor";
...
const normalizeStatus = (value) => {
  if (ALLOWED_STATUS_VALUES.has(value)) return value;
  if (value === 'new') return 'quote';
  if (value === 'scheduled' || value === 'in_progress') return 'waiting_execution';
  if (value === 'completed' || value === 'done') return 'done';
  if (value === 'cancelled') return 'quote';
  return 'quote';
};
...
const normalizePriority = (value) => {
  ...
  if (value === 'not_urgent') return 'normal';
  return 'normal';
};
...
export default function JobForm() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (isEditing) {
        ...
        if (jobData) {
          const normalizedStatus = normalizeStatus(jobData.status);
          const normalizedPriority = jobData.priority === 'normal' ? 'not_urgent' : (jobData.priority || 'not_urgent');
  ...
  const handleChange = (field, value) => {
     setFormData(prev => {
       ...
       // If contact_name is set, it becomes the primary contact
       if (field === 'contact_name' && value) {
         newData.primary_contact_name = formData.contact_name || newData.contact_name;
       }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      submitData.status = normalizeStatus(submitData.status);
      submitData.priority = normalizePriority(submitData.priority);
  ...
  const statusOptions = configs.job_statuses || defaultStatusOptions;
