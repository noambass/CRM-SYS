...
import { Calendar as CalendarIcon, ChevronLeft, ChevronRight, Plus, Eye, Clock, MapPin } from 'lucide-react';
...
export default function Calendar() {
  ...
  const loadJobs = async () => {
    ...
    try {
      ...
      const scheduledQuery = supabase
        .from('jobs')
        .select('id, client_id, title, status, priority, scheduled_at, scheduled_date, scheduled_time, address, city, created_at')
        .eq('owner_id', user.id)
      ...
      const [scheduledRes, unscheduledRes] = await Promise.all([scheduledQuery, unscheduledQuery]);
      if (scheduledRes.error) throw scheduledRes.error;
      if (unscheduledRes.error) throw unscheduledRes.error;

      const merged = [...(scheduledRes.data || []), ...(unscheduledRes.data || [])].map((job) => {
        if (job.scheduled_at) {
  ...
  const getUnscheduledJobs = () => {
    return jobs.filter(job => !getJobScheduledDate(job));
  };
  ...
  const handleDayClick = (day) => {
    if (selectedJobForQuickSchedule) {
      setScheduleData({ date: format(day, 'yyyy-MM-dd'), time: '' });
      setScheduleDialogOpen(true);
    } else {
      setSelectedDay(day);
      navigate(createPageUrl(`JobForm?date=${format(day, 'yyyy-MM-dd')}`));
    }
  };
  ...
  const handleScheduleJob = async () => {
    const jobToSchedule = selectedJobForSchedule || selectedJobForQuickSchedule;
    if (!jobToSchedule || !scheduleData.date || !scheduleData.time) return;
    if (!user) return;
    ...
    try {
      ...
      const { error } = await supabase
        .from('jobs')
        .update(updateData)
        .eq('id', jobToSchedule.id)
        .eq('owner_id', user.id);
      if (error) throw error;

      const updatedJobs = jobs.map(j => j.id === jobToSchedule.id ? {
        ...j,
        ...updateData
      } : j);
      setJobs(updatedJobs);

      // Update day jobs modal if open
      if (selectedDay) {
               ...
               {getUnscheduledJobs().slice(0, 10).map((job) => (
                 <Card 
                   ...
                   className={`border-0 shadow-sm hover:shadow-md transition-all cursor-pointer ${
                     selectedJobForQuickSchedule?.id === job.id ? 'ring-2 ring-amber-500 bg-amber-100' : ''
                   }`}
                 >
                   <CardContent className="p-3">
                     <div className="flex items-start justify-between gap-3">
                       <div 
                         ...
                         onClick={() => setSelectedJobForQuickSchedule(job)}
                       >
                         ...
                         {(job.address || job.city) && (
                           <div className="flex items-center gap-1 text-xs text-slate-500">
                             ...
                             <span className="line-clamp-1">{job.address}{job.city ? `, ${job.city}` : ''}</span>
                           </div>
                         )}
         ...
         <CardContent className="p-6">
          ...
          <div className="flex items-center gap-2 mb-4">
            ...
            <Button
              ...
              size="sm"
              onClick={() => setViewDetails('minimal')}
            ...
            <Button
              ...
              size="sm"
              onClick={() => setViewDetails('detailed')}
              ...
              return (
                <div
                  ...
                  className={`
                    ...
                    ${isDayToday ? 'border-emerald-500 bg-emerald-50' : 'border-slate-200 hover:border-emerald-300 hover:bg-emerald-50/50'}
                    ${!isCurrentMonth ? 'opacity-40' : ''}
                  `}
                >
                  <div className={`text-sm font-medium mb-1 ${isDayToday ? 'text-emerald-600' : 'text-slate-600'}`}>
                    {format(day, 'd')}
                  </div>
                  <div className="space-y-1">
                    {dayJobs.slice(0, viewDetails === 'minimal' ? 1 : 2).map((job) => (
                       <div
                         key={job.id}
                         onClick={(e) => handleJobClick(e, job)}
                         className="text-xs p-1.5 bg-white rounded border border-slate-200 hover:border-emerald-300 hover:shadow-sm transition-all cursor-pointer group"
                       >
                         <div className="font-medium text-slate-700 truncate group-hover:text-emerald-700">{job.title}</div>
                         {viewDetails === 'detailed' && (
                           <>
                             {job.scheduled_time && (
                     ...
                     >
                       ...
                       {job.line_items && job.line_items.length > 0 && (
                         <p className="text-xs text-slate-500 mt-1">
                           {job.line_items.map(item => item.description).join(', ')}
                         </p>
                       )}
        ...
        <Card className="border-0 shadow-sm">
          <CardContent className="p-4">
            ...
            <p className="text-2xl font-bold text-slate-800">
              {jobs.filter((job) => {
                const scheduledDate = getJobScheduledDate(job);
                return scheduledDate && isSameMonth(scheduledDate, currentDate);
              }).length}
            </p>
          </CardContent>
        </Card>
        <Card className="border-0 shadow-sm">
          <CardContent className="p-4">
            ...
            <p className="text-2xl font-bold text-blue-600">
              {jobs.filter((job) => {
                const scheduledDate = getJobScheduledDate(job);
                return scheduledDate && job.status === 'waiting_execution' && scheduledDate > now && isSameMonth(scheduledDate, currentDate);
              }).length}
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="border-0 shadow-sm">
          <CardContent className="p-4">
            ...
            <p className="text-2xl font-bold text-green-600">
              {jobs.filter((job) => {
                const scheduledDate = getJobScheduledDate(job);
                return scheduledDate && job.status === 'done' && isSameMonth(scheduledDate, currentDate);
              }).length}
            </p>
          </CardContent>
        </Card>
